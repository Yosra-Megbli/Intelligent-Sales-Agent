...
- **Overview data-integrity fix:** new `application/metrics_service.py` — single canonical source for every KPI/funnel number shown anywhere (Overview, Leads, Funnel, Contracts, Live Supervision); `DashboardService.get_overview()` now delegates to it instead of computing its own numbers. Removed the fabricated KPI-card deltas and sparklines on the Overview page (`+18.4%` etc. were hardcoded, not derived from any history) — cards now show a neutral "real-time data" indicator instead of inventing a trend. `ConversionFunnel` now renders straight from the backend's canonical funnel stages instead of re-deriving percentages client-side.
...
**Sprint 5 C1 (Live Cockpit SSE Backend) is done:**
- `live/broker.py` — `LiveBroker` ABC with `InProcessAsyncBroker` (asyncio queues per subscriber, clean drop-in seam for Redis pub/sub scaling).
- `live/auth.py` — signed HMAC token generation/verification (`POST /api/live/token` -> `{token, expires_in: 300}`), max-5 concurrent stream slot limiter per API key with Redis & in-memory fallback.
- `api/live_routes.py` — `GET /api/live/stream?token=...` (StreamingResponse `text/event-stream`, 25s heartbeats, 10min max duration, client disconnect detection), `GET /api/keys/health` (honest status reporting).
